...
- Rooklet’s **Android beta** links were approved for publication on 13 September 2026. The overview’s Android button leads to `#android-beta`: join the Google Group, opt into the test, then install from Google Play using the same account. The verified links live in `androidBeta` in its directory data. The **iOS beta** TestFlight link was approved for publication and verified on 15 September 2026. The overview’s matching iOS and Android buttons lead to their respective instruction panels. The URL lives in `iosBeta.joinUrl` in the same directory data; `src/_data/rooklet.js` also exposes that data globally. Keep both platforms described as beta. Both homepage app cards have matching “In beta” badges and “Explore” links only. Keep beta buttons and joining instructions on each app’s page; Coastory’s beta access lives on `https://coastory.app/`. Rooklet’s overview has matching iOS and Android instruction panels, with `#ios-beta` and the preserved `#android-beta` anchors. Source-grounded product copy and privacy facts were obtained from Claude on 11 September 2026. Current public scope includes rough week/month planning and optional habits, meals, and movement. Do not equate a visible habit card with mandatory participation, or promise lossless changes or device-exclusive storage when platform backup can apply. Do not invent release, platform, or pricing claims. All modules, including habits, remain optional to use.
...
Both Coastory previews use **Lauren’s profile**, freshly captured from Coastory 0.9.3 (18) on 12 September 2026. Keep Lauren’s profile for profile screenshots; use the sample data and verified CC0/public-domain photos for other Coastory website and store screenshots. Rooklet’s Today image is a 15 September 2026 simulator capture of 0.3.0 (4), with fictional example tasks and the Today / Plan / Tasks / Notes bar. Asset provenance lives in `images/apps/README.md`.
